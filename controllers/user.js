...
export const recordJourney = async (req, res) => {
  ...
  try {
    ...
    let summary = await Summary.findOneAndUpdate(
      { date: travelDate.setHours(0, 0, 0, 0) },
      {
        $inc: {
          totalAmountCollected: fare - discount,
          totalDiscountGiven: discount,
          [`passengerSummary.${user.passengerType}`]: 1,
        },
      },
...
export const rechargeUserCard = async (req, res, next) => {
  ...
  try {
    ...
    const totalAmount = Number(rechargeAmount) + serviceFee;

    user.zeroCard.balance += Number(rechargeAmount);
    user.zeroCard.lastRecharge = currentDate;

    user.zeroCard.rechargeHistory.push({
      rechargeDate: currentDate,
      rechargeAmount: Number(rechargeAmount),
      rechargeStation: fromStation,
    });
    ...
    let summary = await Summary.findOneAndUpdate(
      { date: currentDate.setHours(0, 0, 0, 0) },
      {
        $inc: {
          ...
          [`serviceFees.${stations[fromStation]}`]: serviceFee,
        },
      },
      { upsert: true, new: true }
    );
    ...
    await user.save();
    res.json({ message: "Card recharged successfully", user: user });
  } catch (err) {
    next(err);
  }
};
